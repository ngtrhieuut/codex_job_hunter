...
Read this entire repository before making implementation decisions. In particular, follow `AGENTS.md`, `docs/PRODUCT_SPEC.md`, `docs/JOB_SCORING.md`, `docs/WORKFLOW.md`, `docs/DATA_MODEL.md`, and `docs/ROADMAP.md`.

## Goal

Implement **Phase 0 and Phase 1** of Codex Job Hunter as a working MVP.

The result must be a usable single-owner internal web app that can ingest technical job opportunities, normalize/deduplicate them, apply hard filters, score them deterministically, and present a ranked/explainable opportunity queue.
...
- Assign clear ownership to subagents such as architecture/database, frontend/dashboard, provider/import, scoring/testing, and security/QA.
...
8. update README with exact local setup and architecture
9. summarize what is implemented, what remains, and any known risks
